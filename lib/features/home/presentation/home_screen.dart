...
class _HomeScreenState extends ConsumerState<HomeScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final isRefereeMode =
        activeMode == UserMode.referee && user?.isVerifiedReferee == true;
                  ...
                  child: CustomScrollView(
                    ...
                    slivers: [
                      ...
                      SliverToBoxAdapter(
                        child: Padding(
                          padding: const EdgeInsets.symmetric(horizontal: 20),
                          child: _buildBookFacilityHeader(
                                context,
                                isRefereeMode,
                              )
                              .animate()
                      ...
                      if (!isRefereeMode) ...[
                        SliverToBoxAdapter(
                          child: _buildHorizontalSportCards(
                            context,
                          ).animate().fadeIn(
                            duration: 600.ms,
                            delay: 400.ms,
  ...
  Widget _buildNotificationButton(BuildContext context, WidgetRef ref) {
    ...
    return unreadCountAsync.when(
      data:
          (count) => _buildGlassIconButtonAnimated(
            Icons.notifications_outlined,
            onTap: () => context.push('/notifications'),
            badge: count > 0 ? count : null,
          ),
      loading:
          () => _buildGlassIconButtonAnimated(
            Icons.notifications_outlined,
            onTap: () => context.push('/notifications'),
          ),
      error:
          (_, __) => _buildGlassIconButtonAnimated(
            Icons.notifications_outlined,
            onTap: () => context.push('/notifications'),
          ),
    );
  ...
  Widget _buildHorizontalSportCards(BuildContext context) {
    return Consumer(
      builder: (context, ref, _) {
        final footballFacilities = ref.watch(
          facilitiesBySportProvider(SportType.football),
        );
        final futsalFacilities = ref.watch(
          facilitiesBySportProvider(SportType.futsal),
        );
        final badmintonFacilities = ref.watch(
          facilitiesBySportProvider(SportType.badminton),
        );
        final tennisFacilities = ref.watch(
          facilitiesBySportProvider(SportType.tennis),
        );
                ...
                children: [
                  Expanded(
                    child: footballFacilities.when(
                      data:
                          (facilities) => _buildCompactSportCard(
                            context,
                            sport: 'Football',
                    ...
                    child: futsalFacilities.when(
                      data:
                          (facilities) => _buildCompactSportCard(
                            ...
                            facilityCount: facilities.length,
                            subtitle:
                                facilities.isNotEmpty ? '1 Court' : '1 Court',
                            price:
                                facilities.isNotEmpty
  ...
  Widget _buildCompactCardContent(
    ...
    final cardHeight =
        isSmallScreen ? 165.0 : 170.0; // Increased to prevent overflow
    final cardPadding = isSmallScreen ? 16.0 : 18.0;
    final iconSize =
        isSmallScreen ? 54.0 : 58.0; // Consistent icon container size
    final iconInnerSize =
        isSmallScreen ? 28.0 : 30.0; // Consistent inner icon size
                        ...
                        Flexible(
                          ...
                          child: Material(
                            ...
                            child: InkWell(
                              onTap:
                                  () =>
                                      context.push('/booking/sport/$sportCode'),
                              borderRadius: BorderRadius.circular(12),
                              child: Container(
                                padding: EdgeInsets.symmetric(
  ...
  Widget _buildGlassIconButtonAnimated(
    IconData icon, {
    VoidCallback? onTap,
    int? badge,
  }) {
  ...
  Widget _buildRefereeStatsSection(
    ...
    return upcomingJobsAsync.when(
      data:
          (upcomingJobs) => allJobsAsync.when(
            data: (allJobs) {
              // Calculate stats
              final completedJobs =
                  allJobs
  ...
  ({IconData icon, Color color}) _getWeatherIcon(String description) {
    ...
    if (desc.contains('thunderstorm') || desc.contains('storm')) {
      return (icon: Icons.thunderstorm_rounded, color: Colors.purple);
    }
    ...
    if (desc.contains('rain') ||
      ...
      return (icon: Icons.grain_rounded, color: Colors.blue);
    }

    // Snow
    if (desc.contains('snow') || desc.contains('sleet')) {
      return (icon: Icons.ac_unit_rounded, color: Colors.lightBlue);
    }
    ...
    if (desc.contains('fog') ||
        desc.contains('mist') ||
        desc.contains('haze')) {
      return (icon: Icons.blur_on_rounded, color: Colors.grey);
    }

    // Cloudy
    if (desc.contains('cloud') || desc.contains('overcast')) {
      return (icon: Icons.cloud_rounded, color: Colors.grey.shade400);
    }

    // Partly cloudy (few clouds)
    if (desc.contains('few') || desc.contains('scattered')) {
      return (icon: Icons.wb_cloudy_rounded, color: Colors.grey.shade300);
    }
    ...
    return (icon: Icons.wb_twilight_rounded, color: Colors.orange);
  }
}
